...
import {
    ...
    Tooltip, CircularProgress, InputBase, Checkbox, Menu, MenuItem, Select, Chip, Avatar
} from '@mui/material';
import {
    Plus, Download, List, GitFork, Search, Filter, MoreVertical,
    ChevronLeft, ChevronRight, Eye, Trash2, ArrowUpDown
} from 'lucide-react';
import { fetchDesignations, deleteDesignation, bulkDeleteDesignations, fetchOrganizations, switchOrganization } from '../utils/api';
import CreateDesignationModal from '../components/Forms/CreateDesignationModal';
...
/* ── Design tokens now come from theme, not hardcoded here ── */
...
export default function Designation() {
    ...
    const [sortConfig, setSortConfig] = useState({ key: 'designationName', direction: 'asc' });
    ...
    const load = React.useCallback(async () => {
        try {
            setLoading(true);
            const response = await fetchDesignations(selectedOrgId);
    ...
    useEffect(() => {
        ...
        const connection = new signalR.HubConnectionBuilder()
            .withUrl(`${config.SOCKET_URL}/hubs/designations`)
        ...
        const start = async () => {
            try {
                ...
                if (!isStopped) {
                    console.log('Connected to DesignationHub');
                    connection.on('ReceiveDesignationUpdate', () => {
                        loadRef.current();
                    });
                } else {
        ...
        return () => {
            ...
            if (connection.state === signalR.HubConnectionState.Connected ||
                ...
                connection.stop().catch(() => { }); // Catch potential abort errors on stop
            }
        };
    }, []); // Run once on mount
    ...
    const handleBulkDelete = async () => {
        if (!window.confirm(`Are you sure you want to delete ${selectedIds.length} designations?`)) return;
        try {
            await bulkDeleteDesignations(selectedIds);
            toast.success('Designations deleted');
            setSelectedIds([]);
            load();
        } catch (error) {
    ...
    const filteredData = data
        .filter(d => {
            const matchesSearch = d.designationName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                d.parentDesignationName?.toLowerCase().includes(searchTerm.toLowerCase());
            const matchesParent = parentFilter === 'All' || d.parentDesignationName === parentFilter || (!d.parentDesignationName && parentFilter === 'None');
    ...
    const parentOptions = ['All', 'None', ...new Set(data.map(d => d.parentDesignationName).filter(p => p))];
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
            ...
            <Box sx={{
                ...
                <Box sx={{
                    ...
                    <Typography sx={{ color: 'text.secondary', fontSize: '12px', ml: 1, mr: 1, whiteSpace: 'nowrap' }}>Parent Designation:</Typography>
                    <Select
                        value={parentFilter}
                ...
                <Stack direction="row" sx={{ bgcolor: 'action.hover', borderRadius: 1.5, p: 0.5 }}>
                    <IconButton
                        ...
                        sx={{
                            ...
                            bgcolor: viewMode === 'list' ? 'rgba(59,130,246,0.15)' : 'transparent',
                            borderRadius: 1.25,
                            p: 1
                        }}
                    >
                    ...
                    <IconButton
                        ...
                        sx={{
                            color: viewMode === 'hierarchy' ? 'primary.main' : 'text.secondary',
                            bgcolor: viewMode === 'hierarchy' ? 'rgba(59,130,246,0.15)' : 'transparent',
                            borderRadius: 1.25,
                            p: 1
                        }}
                    >
                            ...
                            <TableRow sx={{ '& th': { bgcolor: 'action.hover' } }}>
                                ...
                                >
                                    <Stack direction="row" alignItems="center">
                                        Name
                                        <SortIcon columnKey="designationName" />
                                    </Stack>
                                </TableCell>
                                <TableCell
                                    sx={{ color: '#64748b', fontSize: '12px', fontWeight: 600, py: 1.5, borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer' }}
                                    onClick={() => handleSort('parentDesignation')}
                                >
                                    <Stack direction="row" alignItems="center">
                                        Parent Designation
                                        <SortIcon columnKey="parentDesignation" />
                                    </Stack>
                                </TableCell>
                                {isAdminOrAbove && (
                                    <TableCell align="right" sx={{ color: 'text.secondary', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', py: 1.5, borderBottom: '1px solid', borderColor: 'divider' }}>
                                        Action
                                    </TableCell>
                                )}
                            </TableRow>
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                <TableRow key={d.id} sx={{ bgcolor: selectedIds.includes(d.id) ? 'rgba(59,130,246,0.06)' : 'transparent', '&:hover': { bgcolor: 'action.hover' }, '& td': { borderColor: 'divider' } }}>
                                    ...
                                    <TableCell sx={{ color: 'text.primary', fontSize: '13px', py: 1.5 }}>
                                        {d.designationName}
                                    </TableCell>
                                    <TableCell sx={{ color: 'text.secondary', fontSize: '13px', py: 1.5 }}>
            ...
            <Menu
                ...
                PaperProps={{
                    sx: {
                        ...
                        '& .MuiMenuItem-root': {
                            color: 'text.primary', fontSize: '13px',
